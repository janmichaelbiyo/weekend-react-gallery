describe('React Gallery', () => {
  ...
  it('READ: Displays initial Gallery items upon load', () => {
    ...
    cy.get('[data-testid="app"]').should('have.length', 1);

    cy.get('[data-testid="galleryList"]').should('have.length', 1);

    cy.get('[data-testid="galleryItem"]').should('have.length', 6);
  });
  ...
  it('UI: Item Toggles between image and description', () => {
    ...
    cy.contains('[data-testid="galleryItem"]', 'Different...')
      .find('img')
      .should('be.visible');
